...
- OkHttp connection-retry amplification (commit pending, 2026-04-17): Even with pacing correct, Stooq kept blocking the app. Diagnostics showed a single ticker fetch taking 6+ minutes (`watchlist_live_fetch=[KGH#1:07:37:19.728-07:43:36.860]`), a `robots.txt` worker holding the background gate for 18+ minutes, and interceptor `hold` times of 123s — all despite pacing logs showing consistent 3-5s gaps. Root cause: OkHttp's default `retryOnConnectionFailure=true` silently retries across every DNS-resolved route for `stooq.com`. A single app-level call fan-out to 2-3 HTTP attempts, each consuming the full 120s `connectTimeout`, each re-entering `StooqBlockInterceptor` and **each incrementing `consecutiveTimeouts` by one** — so one user-visible call could push the counter from 2 → 5 and trip the 24h block. Fix: added `.retryOnConnectionFailure(false)` in `StooqModule.provideOkHttpClient`, expanded `NotificationWindowRunner.isTerminalStooqFailure` to include `UnknownHostException`/`ConnectException`, added `warnOnAmplifiedHold` canary in `StooqBlockInterceptor` (logs when a single hold exceeds 300s), and two new tests (`StooqOkHttpClientConfigTest`, `incrementsTimeoutStreakExactlyOncePerChainProceedInvocation`). App-level retries still exist in `NotificationWindowRunner.fetchLiveSeriesWithRetries` and `BatchErrorTracker` — these are safe because they pace correctly and detect terminal failures. See `docs/CURRENT_DESIGN.md` §14.9.
  - **How to recognize this pattern in diagnostics:** `stooq_http` gate entries with `hold ≥ ~123000ms` (one 120s timeout) or `hold ≥ ~240000ms` (connect+read timeout combined), combined with a single `watchlist_live_fetch=[TICKER#1:...]` entry spanning multiples of 120s, and `robots_txt` or `premarket_quote` workers that hold the background gate far longer than the normal single-HTTP-call duration. If `hold ≥ 300s`, `warnOnAmplifiedHold` will log a warning and `retryOnConnectionFailure` may have been re-enabled somewhere.
